...
`ticker-follow` is a **simulation-only** paper-trading portfolio for the terminal. No real trades, no real money. It caches daily closes from a user-selected price data provider in SQLite and renders holdings plus Bollinger-band charts.
...
Socket path: `$XDG_RUNTIME_DIR/ticker-follow.sock`, falling back to `/tmp/ticker-follow-<uid>/ticker-follow.sock`. One daemon per user. `tickerd` probes a pre-existing socket on startup and removes it if dead, else refuses to start.
