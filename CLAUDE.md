...
Tests are unit tests and in-process integration tests, run with `cargo test`; there is no harness that spawns real binaries. `tickerd` covers the dispatch surface against an in-memory SQLite and a stubbed provider (`server.rs`), credential file modes (`config.rs`), the rate-limit throttle on tokio's paused clock (`provider/mod.rs`), and ledger file modes (`db.rs`). Anything touching a provider's network path is still validated by running it.
...
- The release profile uses `panic = "abort"` — no unwinding in release builds. That is why `tickerc` restores the terminal from a panic hook rather than a guard object: the hook is the only thing that gets to run.
